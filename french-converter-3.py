"""
This is version 3 of a project developed to simulate the sound changes during
the development of French from Latin. Changes from version 2 allow you to select
a starting point and run from there, thus similuating the evolution of words
borrowed at a later stage of the language.
...
Some notes for use:
    1. You'll notice that the functions for each of the stages of development
       have thre optional parameters: finish, printThis, and showSteps. If
       finish is set to True, it will run that state and all future stages. If
       printThis is set to True, it will print the results of that stage (as
       well as future stages if finish is also True). Lastly, if showSteps is
       set to True, it will print out every individual sound change. Beware this
       one because it will spit out A LOT of data. I mostly incorporated this
       for use in debugging, but you might find it helpful.
...
def setup(word, printThis=True):
    ...
    printif(printThis, new_word)
    return new_word



def from_proto_western_romance(word, finish=True, printThis=True, showSteps=False):

    new_word = setup(word, printThis)
    ...
    # it's difficult to find examples. I decided to include them for now. It
    # also appears that this doesn't happen to single syllable words.
    new_word = re.sub(r"([aeiouɛɔ][bdfgGhjkKlmnprstvwz]+)(!?[eoɛɔ])r$", r"\1r\2", new_word)
    printif(showSteps, new_word)
    ...
    if finish:
        return from_proto_gallo_ibero_romance(new_word, finish, printThis, showSteps)

    return new_word



def from_proto_gallo_ibero_romance(word, finish=True, printThis=True, showSteps=False):

    old_word = from_proto_western_romance(word, False, False)
    new_word = word if word == old_word else setup(word, printThis)
    ...
    if finish:
        return from_early_old_french(new_word, finish, printThis, showSteps)

    return new_word



def from_early_old_french(word, finish=True, printThis=True, showSteps=False):

    old_word = from_proto_gallo_ibero_romance(word, False, False)
    new_word = word if word == old_word else setup(word, printThis)
    ...
    # with the exception of the /a/, /e/, /i/ + /k/, /g/ + /a/ > /j/ rule. I
    # assume that double /k/ or /g/ produces double /tʃa/ or /dʒa/, as well.
    ...
    new_word = re.sub(r"ktʃ", r"ttʃ", new_word)
    new_word = re.sub(r"(?<![aeiǝ])g(?=!?a)", r"dʒ", new_word)
    new_word = re.sub(r"gdʒ", r"ddʒ", new_word)
    ...
    # it should apply to /ʎ/ as well, as I haven't found any direct examples,
    # but through playing around with it a bit, I found that if considered open,
    # it can lead to the combination /waj/ in the future, which I don't believe
    # exists in Modern French (at least based on the "French Orthography"
    # Wikipedia page, which doesn't indicate a possible spelling for it). As
    # such, I will consider it to be closed as well. As of now, I still suspect
    # that affricates are considered open, however.
    new_word = re.sub(r"!e(?!j)(?=([bdfgklmnprstvzðɣ]|tʃ|dʒ|ts|dz)?[jw]?[aeiouɛɔǝ]|$)", r"!ej", new_word)
    new_word = re.sub(r"!o(?!j)(?=([bdfgklmnprstvzðɣ]|tʃ|dʒ|ts|dz)?[jw]?[aeiouɛɔǝ]|$)", r"!ow", new_word)
    new_word = re.sub(r"!a(?![jw]|ɣ[uo])(?=([bdfgklmnprstvzðɣ]|tʃ|dʒ|ts|dz)?[jw]?[aeiouɛɔǝ]|$)", r"!æ", new_word)
    printif(showSteps, new_word)
...
##                new_word += "ǝ"
    ...
    if finish:
        return from_old_french(new_word, finish, printThis, showSteps)

    return new_word



def from_old_french(word, finish=True, printThis=True, showSteps=False):

    old_word = from_early_old_french(word, False, False)
    new_word = word if word == old_word else setup(word, printThis)
    ...
    if finish:
        return from_late_old_french(new_word, finish, printThis, showSteps)

    return new_word



def from_late_old_french(word, finish=True, printThis=True, showSteps=False):

    old_word = from_old_french(word, False, False)
    new_word = word if word == old_word else setup(word, printThis)
    ...
    if finish:
        return from_middle_french(new_word, finish, printThis, showSteps)

    return new_word



def from_middle_french(word, finish=True, printThis=True, showSteps=False):

    old_word = from_late_old_french(word, False, False)
    new_word = word if word == old_word else setup(word, printThis)
    ...
    if finish:
        return from_early_modern_french(new_word, finish, printThis, showSteps)

    return new_word



def from_early_modern_french(word, finish=True, printThis=True, showSteps=False):

    old_word = from_middle_french(word, False, False)
    new_word = word if word == old_word else setup(word, printThis)
    ...
    if finish:
        return from_modern_french(new_word, finish, printThis, showSteps)

    return new_word



def from_modern_french(word, finish=True, printThis=True, showSteps=False):

    old_word = from_early_modern_french(word, False, False)
    new_word = word if word == old_word else setup(word, printThis)
...
def run_tests():
    ...
    passed = True
    for i, j in tests:
        if from_proto_western_romance(i, True, False) != j:
            print(f"failed: {i} > {j}")
            passed = False
